...
address from the subnet's CIDR range. This allows instances to communicate
over a private network without traversing the public internet.
...
output "private_ip" {
  ...
  description = "Private IP assigned from the subnet"
}
```

### Attach VPC and power-cycle to apply
...
### Connect multiple instances to the same subnet
...
| `subnet_id` | String | Yes      | The UUID of the VPC subnet to attach (e.g. `b0825dae-fd86-4d67-8238-c438774da894`). Changing this forces a new resource. |
...
| `private_ip` | String | Private IP address assigned from the subnet CIDR range. |

## Notes

- The private IP is assigned automatically from the subnet's address pool.
- Changing `cloud_id` or `subnet_id` destroys the current attachment and
  creates a new one.
- To detach the subnet, remove the resource block and run `terraform apply`,
  then power-cycle the instance.
- Multiple subnets can be attached to the same instance by creating
  multiple `utho_cloud_vpc` resources.
