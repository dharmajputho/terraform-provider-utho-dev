...
Adds a general storage disk to a Utho Cloud instance. Multiple additional
disks can be added to the same instance by creating multiple
`utho_cloud_storage` resources.
...
| `disk_id`   | String | No       | Disk ID assigned by Utho. Required for update and delete operations. Retrieve from the Utho dashboard after creation. |
| `bus`       | String | No       | Disk bus type. Accepted values: `virtio`, `ide`. Default: `virtio`. |
| `disk_type` | String | No       | Disk role. Accepted values: `Primary`, `Additional`. Only one `Primary` disk is allowed per instance. Default: `Additional`. |
...
| `disk_id` | String | Disk ID assigned by Utho after creation. |

## Workflow

1. Create the resource to add the disk.
2. Note the new disk ID from the Utho dashboard (Storage tab).
3. Add `disk_id` to your configuration.
4. Run `terraform apply` to update the state.
5. Update `bus` or `disk_type` as needed — Terraform will call the update API.
6. Run `terraform destroy` or remove the block to delete the disk.
